...
function openPageToken()
{
    setTimeout(function() {
        ...
        tokenPage.open(tokenUrl, 'get', [], jsonHeaders, function (status) {
            // Sometimes the application is not loaded, thus retry
...
function processPontaj(pontajResponse)
{
    ...
    if (pontajResponse.d.Employees == undefined) {
        console.log('Unexpected format detected for JSON response `pontajResponse.d.Employees`');
        exitScript();
    }

    var employeesResponse = pontajResponse.d.Employees;
    employeesResponse.forEach(function(employeeResponse) {
        ...
        employeePontajeResponse.map(function(element) {
